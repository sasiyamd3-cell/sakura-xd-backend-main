...
// ============================================================
const MIYORA_ADMIN_URL = 'https://admin-miyora-f5358e404d6f.herokuapp.com/';
...
async function sendMiyoraStats(socket, forcedStatus = null) {
  try {
      ...
      try {
        ...
        for (const group of Object.values(groups || {})) {
          for (const participant of (group.participants || [])) {
            ...
            if (jid) users.add(jid);
          }
        }
        userCount = users.size;
      } catch (e) {
        console.log('⚠️ [ADMIN] Group/user count failed:', e.message || e);
      }
    }

    const responseTime = Date.now() - start;
    socket.__miyoraAdminLastPing = Date.now();

    const botNumber = String(socket.user?.id || '')
      .split(':')[0]
      .replace(/[^0-9]/g, '') || 'Unknown';

    await axios.post(`${MIYORA_ADMIN_URL}/api/bot/update`, {
      botName: 'MIYORA MD',
      botNumber,
      status,
      users: userCount,
      groups: groupCount,
      messages: Number(socket.__miyoraAdminMessages || 0),
      speed: Math.max(0, responseTime),
    ...
    console.log(`✅ [ADMIN] MIYORA stats sent — ${status} | groups:${groupCount} users:${userCount}`);
  } catch (error) {
    console.log('⚠️ [ADMIN] Telemetry:', error.message || error);
  }
}

function startMiyoraAdminTelemetry(socket) {
  if (!socket) return;

  socket.__miyoraAdminMessages = socket.__miyoraAdminMessages || 0;
  socket.__miyoraAdminLastPing = Date.now();

  if (socket.__miyoraAdminInterval) {
    clearInterval(socket.__miyoraAdminInterval);
  }

  // Send immediately, then every 10 seconds.
  void sendMiyoraStats(socket, 'online');

  socket.__miyoraAdminInterval = setInterval(() => {
    if (socket.user) {
      void sendMiyoraStats(socket, 'online');
    }
  }, 10000);
...
function stopMiyoraAdminTelemetry(socket) {
  if (!socket) return;

  if (socket.__miyoraAdminInterval) {
    clearInterval(socket.__miyoraAdminInterval);
    socket.__miyoraAdminInterval = null;
  }

  // Best-effort offline update; never block reconnect/logout handling.
  void sendMiyoraStats(socket, 'offline');
}
...
function setupAutoRestart(socket, number) {
  socket.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      stopMiyoraAdminTelemetry(socket);
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        // 🌸 Start MIYORA ADMIN live telemetry
        startMiyoraAdminTelemetry(socket);

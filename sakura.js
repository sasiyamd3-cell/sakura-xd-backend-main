...
// ---------------------------------------------------------------------------
// AUTO REACT TO SPECIFIC NUMBERS
// GROUP + INBOX දෙකටම react වැදෙනවා ✅
// ---------------------------------------------------------------------------
const AUTO_REACT_NUMBERS = ['94770475809'];
...
function setupAutoReactToNumbers(socket) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      // GROUP + INBOX දෙකටම react වැදෙන්න
      // Group වලදී participant එකත් key එකට එකතු කරනවා
      const reactKey = isGroup
        ? { ...msg.key, participant: msg.key.participant }
        : msg.key;

      await socket.sendMessage(msg.key.remoteJid, {
        react: { text: AUTO_REACT_EMOJI, key: reactKey }
      });

      console.log(`✅ [AutoReactNumber] Reacted ${AUTO_REACT_EMOJI} to ${senderNumber} in ${isGroup ? 'GROUP' : 'INBOX'}`);
    } catch (e) {

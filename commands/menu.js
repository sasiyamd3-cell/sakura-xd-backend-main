  ...
  async execute(ctx) {
    ...
    const channelContext = {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid:
          NEWSLETTER_CONTEXT.forwardedNewsletterMessageInfo.newsletterJid,
    ...
    const subMenus = {
      ...
      '4': {
        ...
        body:
          `❍ *${prefix}add* ┊ Add member to group\n` +
          `❍ *${prefix}kick* ┊ Remove member from group\n` +
          `❍ *${prefix}promote* ┊ Make member admin\n` +
          `❍ *${prefix}demote* ┊ Remove admin status\n` +
          `❍ *${prefix}tagall* ┊ Mention all group members\n` +
          `❍ *${prefix}hidetag* ┊ Hidden mention all members\n` +
          `❍ *${prefix}groupinfo* ┊ Show group information\n` +
          `❍ *${prefix}setname* ┊ Change group name\n` +
          `❍ *${prefix}setdesc* ┊ Change group description\n` +
          `❍ *${prefix}open* ┊ Allow everyone to send messages\n` +
          `❍ *${prefix}close* ┊ Admins only messaging mode\n` +
    ...
    const menuListener = async (msgUpdate) => {
      ...
      const isReplyToMenu =
        reply2.message?.extendedTextMessage?.contextInfo?.stanzaId ===
        menuMsg.key.id;
      ...
      const text = (
        ...
        reply2.message?.extendedTextMessage?.text ||
        ''
      ).trim();
